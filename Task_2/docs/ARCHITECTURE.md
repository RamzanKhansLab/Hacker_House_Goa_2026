...
```mermaid
sequenceDiagram
  participant U as VAANI AI browser UI
  participant A as API
  participant S as STT
  participant R as RAG harness
  participant V as Vector/BM25 index
  participant L as LLM
  U->>A: typed query or MIME-normalized audio upload
...
The browser uses `MediaRecorder.isTypeSupported()` to choose a supported WebM/Ogg recorder MIME type and uploads a matching filename. The API strips safe MIME parameters (for example, `audio/webm;codecs=opus` becomes `audio/webm`) before applying its allow-list and validates that the filename extension matches the media type. Sarvam REST is not streaming: the UI shows listening locally, then shows the returned backend transcript before presenting the existing RAG result.
